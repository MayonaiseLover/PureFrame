...
def test():
    ...
    original_apply = pureframe.pipeline.render.apply.apply_censoring
    def mocked_apply_censoring(input_path, output_path, frame_to_boxes, config, settings):
        print("Box for frame 150:", frame_to_boxes.get(150))
        original_apply(input_path, output_path, frame_to_boxes, config, settings)
